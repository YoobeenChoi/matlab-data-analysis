...
raw_data = readtable('pilot_data_final.xlsx', 'Sheet', 'raw data');
raw_data.Properties.VariableNames
cleaned_data = table();

% Exclude unnecessary variables
excluded_events = {'time', 'Water', 'VoidOff'};
...
ocp_on_idx = find(strcmp(raw_data.Var1, 'OCP_on')); % Indices where 'OCP_on' appears in the first column of raw_data
preobjon_idx = find(strcmp(raw_data.Var1, 'PreObjOn'));
objon_idx = find(strcmp(raw_data.Var1, 'ObjOn'));

ObjOn(1:15) = raw_data.Var2(preobjon_idx(1:15));  % First 15 rows for Lap 0
for i = 1:8 % For Laps 1 to 8, assign ObjOn times in groups of 4 rows each from raw_data
    ObjOn(15 + (i-1)*4 + 1 : 15 + i*4) = raw_data.Var2(objon_idx((i-1)*4 + 1 : i*4));
end
ObjOn(end-14:end) = raw_data.Var2(preobjon_idx(end-14:end));  % Last 15 rows for Lap 9
...
ocp_on_idx = find(strcmp(raw_data.Var1, 'OCP_on'));
...
% ChoiceOn
choiceon_rows = strcmp(raw_data.Var1, 'ChoiceOn'); % Find rows in raw_data where the first column is 'ChoiceOn'
...
new_table.ChoiceOn(16:47) = ChoiceOn;
...
Correct_Num(lap1_8_idx) = decision_values(1:length(lap1_8_idx)); 
...
ObjOff(1:15) = raw_data.Var2(preobjoff_idx_0(1:15)); 
preobjoff_idx_9 = find(strcmp(raw_data.Var1(ocp_on_idx(2):end), 'PreObjOff')) + ocp_on_idx(2) - 1;
ObjOff(end-14:end) = raw_data.Var2(preobjoff_idx_9(1:15));  
objoff_idx = find(strcmp(raw_data.Var1, 'ObjOff'));
lap1_8_idx = find(new_table.Lap >= 1 & new_table.Lap <= 8);  % Find rows for Lap 1 to 8
ObjOff(lap1_8_idx) = raw_data.Var2(objoff_idx(1:length(lap1_8_idx)));  % Record ObjOff values for Lap 1 to 8
...
lap_end_rows = strcmp(raw_data.Var1, 'LapEnd');
nav_end_values = raw_data.Var2(lap_end_rows);
lap_trial_4_idx = find(new_table.Lap_Trial == "4");
NavEnd(lap_trial_4_idx) = nav_end_values(1:length(lap_trial_4_idx));
...
lap_end_idx = find(strcmp(raw_data.Var1, 'LapEnd'), 1, 'last');

choice_a_idx = find(strcmp(raw_data.Var1(lap_start_idx:lap_end_idx), 'ChoiceA')) + lap_start_idx - 1;
choice_b_idx = find(strcmp(raw_data.Var1(lap_start_idx:lap_end_idx), 'ChoiceB')) + lap_start_idx - 1;
    ...
    if new_table.Lap(i) >= 1 && new_table.Lap(i) <= 8  % Only for Lap 1 to 8
        if new_table.Correct_Num(i) ~= 2 && Choice_txt(i) == ""  % Skip 'missing' and already filled
            if choice_counter <= length(choice_idx)  % Ensure the counter doesn't exceed choice_idx length
                if strcmp(raw_data.Var1(choice_idx(choice_counter)), 'ChoiceA')
                    Choice_txt(i) = "A";
                elseif strcmp(raw_data.Var1(choice_idx(choice_counter)), 'ChoiceB')
                    Choice_txt(i) = "B";
                end
...
% ITIEnd
ITIEnd = NaN(height(new_table), 1); 
trial_end_rows = strcmp(raw_data.Var1, 'TrialEnd');
iti_end_values = raw_data.Var2(trial_end_rows);
lap1_8_idx = find(new_table.Lap >= 1 & new_table.Lap <= 8);  % Find rows for Lap 1 to 8 in the new_table
ITIEnd(lap1_8_idx) = iti_end_values(1:length(lap1_8_idx));  % Assign ITIEnd values to Lap 1 to 8
new_table.ITIEnd = ITIEnd;
...
ocp_on_idx = find(strcmp(raw_data.Var1, 'OCP_on'));
ocp_off_idx = find(strcmp(raw_data.Var1, 'OCP_off'));
...
% Find ButtonA events between the first OCP_on and OCP_off for Lap 0
button_a_idx_0 = find(strcmp(raw_data.Var1(start_idx_0:end_idx_0), 'ButtonA')) + start_idx_0 - 1;

% Find ButtonA events between the second OCP_on and OCP_off for Lap 9
button_a_idx_9 = find(strcmp(raw_data.Var1(start_idx_9:end_idx_9), 'ButtonA')) + start_idx_9 - 1;
...
for i = 1:length(button_a_idx_0)
    button_a_time = raw_data.Var2(button_a_idx_0(i));  % Get the ButtonA time
...
for i = 1:length(button_a_idx_9)
    button_a_time = raw_data.Var2(button_a_idx_9(i));  % Get the ButtonA time
...
new_table = new_table(:, {'Lap', 'NavStart', 'Trial', 'Lap_Trial', 'Context_txt', 'Context_Num', 'Direction', 'Location', 'Association', 'Obj_ID', 'ObjOn', 'ChoiceOn', 'Choice_Num', 'Choice_txt', 'Correct_Num', 'Correct_txt', 'isTimeout', 'RT', 'ObjOff', 'ITIEnd', 'NavEnd'});
new_table

% Specify the filename for the new Excel file
filename = 'new_table_output.xlsx';
